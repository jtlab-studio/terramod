...
const Canvas: React.FC = () => {
  ...
  useEffect(() => {
    const updateDimensions = () => {
      if (containerRef.current) {
        setDimensions({
          width: Math.max(containerRef.current.offsetWidth, CANVAS_MIN_WIDTH),
  ...
  const getDefaultArguments = (service: ServiceDefinition): Record<string, any> => {
    ...
    if (service.resource_type === 'aws_vpc') defaults.cidr_block = '10.0.0.0/16';
    else if (service.resource_type === 'aws_subnet') defaults.cidr_block = '10.0.1.0/24';
    else if (service.resource_type === 'aws_instance') {
      defaults.ami = 'ami-0c55b159cbfafe1f0';
      defaults.instance_type = 't3.micro';
    } else if (service.resource_type === 'aws_s3_bucket') defaults.bucket = `my-bucket-${Date.now()}`;
    return defaults;
  };
  ...
  const handleStageDrop = useCallback((e: React.DragEvent) => {
    ...
    try {
      ...
      const x = snapToGrid((e.clientX - stageBox.left - viewport.x) / viewport.zoom);
      const y = snapToGrid((e.clientY - stageBox.top - viewport.y) / viewport.zoom);
  ...
  const calculateOrthogonalPath = (startX: number, startY: number, endX: number, endY: number): number[] => {
  ...
  const renderGrid = () => {
    ...
    for (let i = 0; i < dimensions.width / GRID_SIZE + 10; i++) {
      lines.push(<Line key={`v-${i}`} points={[i * GRID_SIZE, 0, i * GRID_SIZE, dimensions.height]} stroke={gridColor} strokeWidth={1} opacity={0.3} listening={false} />);
    }
    for (let i = 0; i < dimensions.height / GRID_SIZE + 10; i++) {
      lines.push(<Line key={`h-${i}`} points={[0, i * GRID_SIZE, dimensions.width, i * GRID_SIZE]} stroke={gridColor} strokeWidth={1} opacity={0.3} listening={false} />);
    }
    return lines;
  };
  ...
  const renderConnections = () => {
    return connections.map((connection) => {
      ...
      const isDragging = draggingResourceId === connection.sourceId || draggingResourceId === connection.targetId;
      ...
      const points = calculateOrthogonalPath(sourcePos.x + 160, sourcePos.y + 40, targetPos.x, targetPos.y + 40);
  ...
  return (
    <div ref={containerRef} className="flex-1 bg-gray-900 overflow-hidden relative" onDrop={handleStageDrop} onDragOver={handleDragOver} onDragLeave={() => setDropFeedback(null)}>
      ...
      <div className="absolute bottom-4 left-4 z-10 bg-gray-800 border border-gray-700 px-3 py-2 rounded text-xs text-gray-400">
        ...
        <div>• Drag card from anywhere to move</div>
        <div>• Click center to select resource</div>
        <div>• Click line to select connection</div>
        <div>• Drag dots to connect resources</div>
        <div>• DEL to delete selected item</div>
      </div>

      {dropFeedback && (
        <div className="absolute z-10 w-40 h-20 border-2 border-dashed border-gray-500 bg-gray-500 opacity-20 rounded pointer-events-none"
          style={{ left: dropFeedback.x * viewport.zoom + viewport.x, top: dropFeedback.y * viewport.zoom + viewport.y }} />
      )}

      <Stage ref={stageRef} width={dimensions.width} height={dimensions.height} onWheel={handleWheel}
        onClick={(e) => { if (e.target === e.target.getStage()) { setSelectedId(null); setConnectionDraft(null); } }}
        onMouseMove={(e) => {
          if (connectionDraft) {
        ...
        scaleX={viewport.zoom} scaleY={viewport.zoom} x={viewport.x} y={viewport.y} draggable={false}>
        ...
        <Layer>
          {renderConnections()}
          {connectionDraft && (
            <Line points={calculateOrthogonalPath(connectionDraft.startX, connectionDraft.startY, connectionDraft.currentX, connectionDraft.currentY)}
              stroke="#9CA3AF" strokeWidth={2} dash={[5, 5]} opacity={0.7} listening={false} />
          )}
        </Layer>
        ...
        <Layer>
          {resources.map((resource) => (
            <ResourceCard key={resource.id} resource={resource}
              onConnectionStart={(resourceId, x, y) => setConnectionDraft({ sourceId: resourceId, startX: x, startY: y, currentX: x, currentY: y })}
              onConnectionEnd={(targetId) => {
                if (connectionDraft && targetId && targetId !== connectionDraft.sourceId) {
                  addConnection({
                    id: generateId('conn'),
              ...
              onDragMove={(resourceId, x, y) => setDragPositions(prev => { const m = new Map(prev); m.set(resourceId, { x, y }); return m; })}
              onDragEnd={() => { setDraggingResourceId(null); setDragPositions(prev => { const m = new Map(prev); m.delete(resource.id); return m; }); }}
